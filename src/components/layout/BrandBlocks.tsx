...
export function BrandBlocks({ className, variant = 'corner' }: BrandBlocksProps) {
  if (variant === 'stack') {
    return (
      <div className={cn('relative w-14 h-14 pointer-events-none select-none', className)} aria-hidden="true">
        ...
        <motion.span animate={floatB.animate} transition={floatB.transition} className="absolute top-0 right-0 w-6 h-6 bg-golden-500" />
      </div>
    )
  }
  ...
  return (
    <div className={cn('relative w-16 h-16 pointer-events-none select-none', className)} aria-hidden="true">
      ...
      <motion.span animate={floatB.animate} transition={floatB.transition} className="absolute bottom-6 left-9 w-6 h-6 bg-golden-500" />
    </div>
  )
}
