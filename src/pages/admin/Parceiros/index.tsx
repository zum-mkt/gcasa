...
function PartnerForm({ open, onClose, item }: { open: boolean; onClose: () => void; item: Partner | null }) {
  ...
  const { register, handleSubmit, reset, setValue, watch, formState: { errors } } = useForm<FormData>({
    ...
    defaultValues: { active: true, order_index: 0 },
  })
  ...
  useEffect(() => {
    if (item) {
      ...
      reset({ active: true, order_index: 0 })
    }
  }, [item, reset])
  ...
  const mutation = useMutation({
    mutationFn: async (data: FormData) => {
      ...
      if (isEdit) {
        const { error } = await supabase.from('partners').update(payload).eq('id', item.id)
        if (error) throw error
      } else {
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ['partners-home'] })
    ...
    >
      <form className="space-y-5">
        ...
        <div className="grid grid-cols-2 gap-4">
          <ImageUpload
            label="Logo"
            hint="Fundo transparente (PNG/SVG). Exibido no card e no modal."
          ...
          <ImageUpload
            ...
            hint="Foto da empresa/produto (JPG/WebP). Usada como banner no modal."
        ...
        <div className="grid grid-cols-2 gap-4">
          <Input label="Categoria" {...register('category')} placeholder="Ex: Materiais, Tintas, Pisos…" />
          <Input label="Site" error={errors.site_url?.message} {...register('site_url')} placeholder="https://exemplo.com" />
        </div>

        <Textarea label="Descrição" rows={3} {...register('description')} placeholder="Conte sobre a empresa, produtos ou serviços oferecidos…" />

        {/* Contatos */}
        <div>
          <p className="text-xs font-semibold text-gray-500 uppercase tracking-widest mb-3">Contato</p>
          <div className="grid grid-cols-2 gap-4">
            <Input label="Telefone" {...register('phone')} placeholder="+55 16 9999-0000" />
            <Input label="WhatsApp" {...register('whatsapp')} placeholder="5516999990000" />
            <Input label="Instagram" {...register('instagram')} placeholder="@empresa" />
            <Input label="Facebook" {...register('facebook')} placeholder="facebook.com/empresa" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <Select
            label="Estado"
            placeholder="UF"
            options={[{ value: '', label: '—' }, ...brazilStates.map(s => ({ value: s, label: s }))]}
            {...register('state')}
          />
        </div>
...
export default function AdminParceiros() {
  ...
  const columns: ColumnDef<Partner>[] = [
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 bg-gray-50 border border-gray-100 overflow-hidden flex-shrink-0 flex items-center justify-center">
              {p.logo_url
            ...
            <div>
              ...
              {p.category && <p className="text-xs text-primary-500 font-medium">{p.category}</p>}
              {p.city && <p className="text-xs text-gray-400">{[p.city, p.state].filter(Boolean).join(', ')}</p>}
            </div>
          </div>
        )
      },
    },
    {
      ...
      header: 'Site',
      cell: ({ row }) => row.original.site_url
        ? <a href={row.original.site_url} target="_blank" rel="noreferrer" className="text-xs text-primary-600 hover:underline truncate max-w-[200px] block">{row.original.site_url}</a>
    ...
    },
    {
      ...
      cell: ({ row }) => row.original.description
        ? <span className="text-xs text-gray-500 line-clamp-1 max-w-[200px]">{row.original.description}</span>
        : <span className="text-gray-300 text-xs">—</span>,
    },
    {
      accessorKey: 'active',
      header: 'Status',
      cell: ({ row }) => <Badge variant={row.original.active ? 'success' : 'default'}>{row.original.active ? 'Ativo' : 'Inativo'}</Badge>,
    },
    {
      id: 'actions',
      header: '',
      cell: ({ row }) => (
        <div className="flex items-center gap-1 justify-end">
          ...
          <Button variant="danger-ghost" size="icon-sm" onClick={() => setDeletingId(row.original.id)}>
            <Trash2 size={14} />
          </Button>
        </div>
      ),
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description={`${data.length} parceiro${data.length !== 1 ? 's' : ''} cadastrado${data.length !== 1 ? 's' : ''}`}
        actions={
